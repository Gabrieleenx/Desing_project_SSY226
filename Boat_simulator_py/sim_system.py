...
boat_pos_y = 6400188 # in meters, northing
boat_pos_x = 677214 # in meters, esting
boat_heading = 0*np.pi # headig in radians. 0 north, + gives west, i.e. right hand rule. 
...
gps_zone_num = 32 # sweden
gps_zone_letter = 'V' # Sweden
gps_est_std = 0.4 # standard deviation noise added to easting, defined in meters. 
gps_north_std = 0.4
imu_yaw_std = 0.05 # std in radians

# waypoints 
waypoints_global = np.array([[0,1,3,6,9,9,2], [2,2.5,3,2,0,-4,-10]])+np.array([[boat_pos_x],[boat_pos_y]]) # x , y 
num_waypoints = 7
send_way_t = 5 # seconds between sending new waypoints. 
# mutex
...
def start_microservice():
    ...
    msg_pose_data = porpoise_message_set.opendlv_proxy_Pose_data()
    msg_mission = porpoise_message_set.opendlv_proxy_Mission()
    msg_imu_orienation = porpoise_message_set.opendlv_logic_sensation_Orientation()
    msg_gps = porpoise_message_set.opendlv_grove_GPSReading()
    ...
    while True:
        ...
        lat, lon = utm.to_latlon(boat_pos_x_noise, boat_pos_y_noise, gps_zone_num, gps_zone_letter)
        
        msg_gps.utc_time = str(datetime.utcnow())
        msg_gps.latitude = str(lat)
        msg_gps.longitude = str(lon)

        msg_gps.ns_indicator = 'N'
        msg_gps.ew_indicator = 'E'
        msg_gps.pos_fix_indicator = '0'
        msg_gps.satellites_used = '10'
        msg_gps.msl_altitude = '0'


        # Send message on id 
        session.send(5402, msg_gps.SerializeToString())

        # IMU

        msg_imu_orienation.roll = 0.0
        msg_imu_orienation.pitch = 0.0
        msg_imu_orienation.yaw = boat_heading + np.random.normal(0, imu_yaw_std)
        session.send(1118, msg_imu_orienation.SerializeToString())

        if iter%(send_way_t/delta_T) == 0:
